...
class MasksAnalyzer:
    def __init__(self, model_name: str, lesion_masks_path: str, variance_masks_path: str):
        ...
        :param lesion_masks_path: path to a file with original lesion masks
        :param variance_masks_path: path to a file with generated variance masks
        """
        self.model_name = model_name
        self.lesion_masks_path = lesion_masks_path
        self.variance_masks_path = variance_masks_path
        self.lesion_mask_names = self.load_mask_names(self.lesion_masks_path)
        self.variance_mask_names = self.load_mask_names(self.variance_masks_path)
        self.metrics = {
            "dice_coefficient": [],
            "hausdorff_distance": [],
            "jaccard_index": [],
    ...
    def perform_analysis(self, save_to_json: bool = False):
        ...
        for lesion_mask_name, variance_mask_name in zip(self.lesion_mask_names, self.variance_mask_names):
            path_to_lesion_mask = self.lesion_masks_path + lesion_mask_name
            path_to_variance_mask = self.variance_masks_path + variance_mask_name
            similarity_comparer = SimilarityComparer(path_to_lesion_mask, path_to_variance_mask)
            similarity_comparer.perform_analysis()
            metrics = similarity_comparer.get_metrics()
    ...
    @staticmethod
    def load_mask_names(path_to_masks: str) -> List[str]:
        """
        Loads mask paths from specified path.
        :param path_to_masks: a path that mask paths will be loaded from
        :return: a list of sorted mask paths
        """
        return sorted(os.listdir(path_to_masks))
